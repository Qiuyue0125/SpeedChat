...
BINARY="$BUILD_DIR/$BUILD_TYPE/suliao"
APPDIR="$SCRIPT_DIR/suliao.AppDir"
APP_NAME="suliao.AppImage"
...
cp "$SCRIPT_DIR/src/appimage/suliao.desktop"     "$APPDIR/"
cp "$SCRIPT_DIR/src/appimage/suliao.png"         "$APPDIR/"
cp "$SCRIPT_DIR/src/appimage/suliao.png"         "$APPDIR/.DirIcon"
cp "$SCRIPT_DIR/src/appimage/suliao.png"         "$APPDIR/usr/share/icons/hicolor/256x256/apps/"
cp "$SCRIPT_DIR/src/appimage/AppRun"            "$APPDIR/"
chmod +x "$APPDIR/AppRun"
sed -i 's/\r$//' "$APPDIR/suliao.desktop" "$APPDIR/AppRun" 2>/dev/null || true
...
LINUXDEPLOY_OUTPUT=$("$LINUXDEPLOY" \
    --appdir "$APPDIR" \
    --executable "$APPDIR/usr/bin/suliao" \
    --library "${QT_PREFIX}/plugins/platforms/libqxcb.so" \
    --desktop-file "$APPDIR/suliao.desktop" \
    --icon-file "$APPDIR/suliao.png" \
    --plugin qt 2>&1)
LINUXDEPLOY_RC=$?
